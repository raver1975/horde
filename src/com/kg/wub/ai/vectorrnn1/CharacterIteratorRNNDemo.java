package com.kg.wub.ai.vectorrnn1;
...
//    private Vector[] validCharacters;
	 ...
	 */
	public CharacterIteratorRNNDemo(Segment[] segs, Charset textFileEncoding, int miniBatchSize, int exampleLength,
                                    Random rng) throws IOException {
		//if( !new File(textFilePath).exists()) throw new IOException("Could not access file (does not exist): " + textFilePath);
		if( miniBatchSize <= 0 ) throw new IllegalArgumentException("Invalid miniBatchSize (must be >0)");
		this.exampleLength = exampleLength;
		this.miniBatchSize = miniBatchSize;
		this.segs=new Vector[segs.length];
		for (int i=0;i<segs.length;i++){
			this.segs[i]=new Vector(segs[i]);
		}
		this.rng = rng;
		...
		for( int i=0,b=0; i<segs.length; i++ ) {
			Vector ins = new Vector(segs[i]);
			if (charToIdxMap.get(ins) == null) {
				charToIdxMap.put(new Vector(segs[i]), b++);
			}
		}
//		validCharacters=new Vector[charToIdxMap.size()];
//		int j=0;
//		for (Vector c:charToIdxMap.keySet()){
//			validCharacters[j++]=c;
//		}
...
//	}
	...
	public Vector getRandomCharacter(){
		return segs[(int) (rng.nextDouble()*segs.length)];
	}
	...
	public DataSet next(int num) {
        ...
        for( int i=0; i<currMinibatchSize; i++ ){
            ...
            int currCharIdx = charToIdxMap.get(segs[startIdx]);	//Current input
            int c=0;
            for( int j=startIdx+1; j<endIdx; j++, c++ ){
                int nextCharIdx = charToIdxMap.get(segs[j]);		//Next character to predict
	...
	public int totalExamples() {
		return (segs.length-1) / miniBatchSize - 2;
	}

	public int inputColumns() {
		return Vector.attLength;
	}
    ...
    private void initializeOffsets() {
        ...
        int nMinibatchesPerEpoch = (segs.length - 1) / exampleLength - 2;   //-2: for end index, and for partial example
